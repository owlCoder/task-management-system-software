 ...
 * - `origin`: List of allowed origins for cross-origin requests, set through `CORS_ORIGIN` environment variable. Defaults to an empty string if not provided.
 * - `methods`: List of allowed HTTP methods for cross-origin requests, set through `CORS_METHODS` environment variable. Defaults to ['GET', 'POST', 'PUT', 'PATCH', 'DELETE'] if not provided.
